...
class AccountingServiceMap implements AccountingService {
    ...
    constructor() {
        this._accounts.set("admin", {userName: "admin", password: "$2a$10$N82yjQXHhblkQXxALgB0E.zThFGeEtNGjIV2qnQcr0wIYpTogRm/u", role: "ADMIN"}); // pass admin1
        this._accounts.set("user",  {userName: "user",  password: "$2a$10$HaY2kh2ZVxffEzX1TcuNZ.kbcIrWbLYBHjkdGwzTtNm4Ir72j1Gu.", role: "USER"}); // pass user1
    }

    login(loginData: LoginData): string {
        const account = this._accounts.get(loginData.email);
        
        if (compareSync(loginData.password, account.password)) {
