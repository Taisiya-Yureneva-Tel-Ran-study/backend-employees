...
test("invalid user", async() => {
    const res = await supertest(app).post("/login").send(invalidUser);
    assert.equal(res.statusCode, INVALID_DATA_CODE);
})

test("successful admin login", async() => {
    const res = await supertest(app).post("/login").send(testAdmin);
    adminToken = res.body.accessToken;
    assert.equal(res.statusCode, OK_CODE);
})

test("successful user login", async() => {
    const res = await supertest(app).post("/login").send(testUser);
    userToken = res.body.accessToken;
    assert.equal(res.statusCode, OK_CODE);
})

test("successful dev login", async() => {
    const res = await supertest(app).post("/login").send(testDev);
...
test("cannot set employee unauthorized", async () => {
    const res = await supertest(app).post("/employees").send(testEmployee1);
    assert.equal(res.statusCode, UNAUTHORIZED_CODE);
})

test("set employee with admin auth", async () => {
    const res = (await supertest(app).post("/employees").set("Authorization", "Bearer " + adminToken).send(testEmployee1));
    assert.equal(res.body.id, testEmployee1.id);
    assert.equal(res.statusCode, OK_CODE);
})

test("cannot set invalid employee data with admin auth", async () => {
    const res = (await supertest(app).post("/employees").set("Authorization", "Bearer " + adminToken).send(invalidEmployee));
    assert.equal(res.statusCode, INVALID_DATA_CODE);
})

test("cannot set employee data with user auth", async () => {
    const res = (await supertest(app).post("/employees").set("Authorization", "Bearer " + userToken).send(testEmployee2));
    assert.equal(res.statusCode, FORBIDDEN_CODE);
})

// Test get employee by id
test("get employee data with user auth", async () => {
    const res = (await supertest(app).get(`/employees/${testEmployee1.id}`).set("Authorization", "Bearer " + userToken));
    assert.equal(res.body.id, testEmployee1.id);
    assert.equal(res.statusCode, OK_CODE);
})

test("not get employee data with developer auth", async () => {
    const res = (await supertest(app).get(`/employees`).query({id: testEmployee1.id}).set("Authorization", "Bearer " + devToken));
    assert.equal(res.statusCode, FORBIDDEN_CODE);
})

test("not get employee data without any auth", async () => {
    const res = (await supertest(app).get(`/employees/${testEmployee1.id}`));
    assert.equal(res.statusCode, UNAUTHORIZED_CODE);
})

// Update employee
test("update employee data with admin auth", async () => {
    const res = (await supertest(app).patch(`/employees/${testEmployee1.id}`).set("Authorization", "Bearer " + adminToken).send({salary: 40000}));
    assert.equal(res.body.salary, 40000);
    assert.equal(res.statusCode, OK_CODE);
})

test("cannot update employee data with user auth", async () => {
    const res = (await supertest(app).patch(`/employees/${testEmployee1.id}`).set("Authorization", "Bearer " + userToken).send({salary: 0}));
    assert.equal(res.statusCode, FORBIDDEN_CODE);
})
...
test("get employees with user auth", async () => {
    const res = await supertest(app).get("/employees").set("Authorization", "Bearer " + userToken);
    assert.notEqual(res.body.length, 0);
    assert.equal(res.statusCode, OK_CODE);
})

test("get employees with admin auth", async () => {
    const res = await supertest(app).get("/employees").set("Authorization", "Bearer " + adminToken);
    assert.notEqual(res.body.length, 0);
    assert.equal(res.statusCode, OK_CODE);
})

test("cannot get employees with developer auth", async () => {
    const res = await supertest(app).get("/employees").set("Authorization", "Bearer " + devToken);
    assert.equal(res.statusCode, FORBIDDEN_CODE);
})

// Delete employee
test("cannot delete employees with user auth", async () => {
    const res = await supertest(app).delete(`/employees/${testEmployee1.id}`).set("Authorization", "Bearer " + userToken);
    assert.equal(res.statusCode, FORBIDDEN_CODE);
})

test("delete employee with admin auth", async () => {
    const res = await supertest(app).delete(`/employees/${testEmployee1.id}`).set("Authorization", "Bearer " + adminToken);
    assert.equal(res.body.id, testEmployee1.id);
    assert.equal(res.statusCode, OK_CODE);
})
